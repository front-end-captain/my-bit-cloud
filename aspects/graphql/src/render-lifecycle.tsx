...
import type { RenderPlugin, SsrSession } from "@unknown/react.rendering.ssr";
...
export class GraphqlRenderPlugins
  implements RenderPlugin<RenderContext, { state?: NormalizedCacheObject }>
{
  ...
  private BrowserGqlProvider = ({
    ...
    if (!renderCtx?.client)
      throw new TypeError("GQL client is not initialized, make sure `.browserInit()` executes");

    return (
      <this.graphqlUI.getProvider client={renderCtx?.client}>{children}</this.graphqlUI.getProvider>
    );
  };
...
function ServerGqlProvider({
  ...
  if (!renderCtx?.client)
    throw new TypeError(
      "GQL client has not been initialized during SSR, make sure `.serverInit()` executes",
    );
