...
import { Harmony, Slot, SlotRegistry } from "@unknown/harmony";
import express, { Express } from "express";
import { graphqlHTTP } from "express-graphql";
import { Port } from "@unknown/network";
...
import { GraphqlMainRunTime } from "./graphql.aspect";
...
export class GraphqlMain {
  ...
  async createServer(options: GraphQLServerOptions) {
    ...
    const remoteSchemas = await createRemoteSchemas(
      options.remoteSchemas || this.graphQLServerSlot.values(),
    );
    ...
    app.use(
      ...
      graphqlHTTP((request, res, params) => ({
        customFormatErrorFn: (err) => {
          console.error("graphql got an error during running the following query:", params);
  ...
  async listen(port?: number, server?: Server, app?: Express) {
    ...
    subServer.listen(serverPort, () => {
      console.info(`API Server over HTTP is now running on http://localhost:${serverPort}`);
      console.info(
  ...
  private async createSubscription(options: GraphQLServerOptions, port: number) {
    ...
    websocketServer.listen(port, () =>
      console.debug(`Websocket Server is now running on http://localhost:${port}`),
    );
  ...
  static slots = [
    Slot.withType<Schema>(),
    Slot.withType<GraphQLServer>(),
    Slot.withType<PubSubSlot>(),
  ];
  ...
  static runtime = GraphqlMainRunTime;
  static dependencies = [];
  ...
  static async provider(
    ...
    const graphqlMain = new GraphqlMain(config, moduleSlot, context, graphQLServerSlot, pubSubSlot);
    return graphqlMain;
  }
}
