...
import { ExpressAspect, ExpressMainRuntime } from "./express.aspect";
...
export class ExpressMain {
  static runtime = ExpressMainRuntime;
  ...
  createApp(expressApp?: Express, options?: { disableBodyParser: true }): Express {
    ...
    app.use((req, res, next) => {
      ...
      console.debug(`express got a request to a URL: ${req.url}', headers:`, req.headers);
      return next();
    });
  ...
  static async provider(
    ...
    return new ExpressMain(config, routeSlot, middlewareSlot);
  }
}
