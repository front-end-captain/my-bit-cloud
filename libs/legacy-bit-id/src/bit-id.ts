...
import { versionParser, isHash, LATEST_VERSION } from "@unknown/component-version";
...
export default class BitId {
  ...
  static getValidScopeName(scope: string) {
    ...
    if (!cleanName) {
      ...
      throw new Error(
        "scope name created by directory name have to contains at least one character or number",
      );
    }
    return cleanName;
  }
