...
export class Http implements Network {
  ...
  static async getNetworkConfig(): Promise<NetworkConfig> {
    ...
    const strictSSL =
      obj[CFG_NETWORK_STRICT_SSL] ?? obj["network.strict_ssl"] ?? obj[CFG_PROXY_STRICT_SSL];
    const networkConfig = {
  ...
  async pushToCentralHub(
    ...
    const route = "api/put";
    logger.debug(
      `Http.pushToCentralHub, started. url: ${
        this.url
      }/${route}. total objects ${objectList.count()}`,
    );
  ...
  async fetch(ids: string[], fetchOptions: FETCH_OPTIONS): Promise<ObjectItemsStream> {
    ...
    const res = await fetch(`${this.url}/${route}`, opts);
    logger.debug(
      `Http.fetch got a response, ${scopeData}, status ${res.status}, statusText ${res.statusText}`,
    );
    await this.throwForNonOkStatus(res);
  ...
  private async graphClientRequest(
    ...
    logger.debug(
      `http.graphClientRequest, scope "${this.scopeName}", url "${this.url}", query ${query}`,
    );
    try {
  ...
  async graph(bitId?: BitId): Promise<DependencyGraph> {
    ...
    const nodes = graph.nodes.map((node) => ({
      idStr: node.id,
      bitId: new BitId(node.component.id),
    }));
...
export function getAuthDataFromHeader(
