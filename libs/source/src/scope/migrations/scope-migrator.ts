...
export default async function migrate(
  ...
  if (R.isEmpty(migrations)) {
    const noMigrationMsg =
      "there are no migrations to run, leaving the scope as is with no changes";
...
const _runOneMigrationForObject =
  ...
  (migration: MigrationDeclaration) => {
    logger.debug(
      `running migration: ${migration.name} on object ${rawObject.ref} (${rawObject.id})`,
    );
    if (globalVerbose)
      console.log(
        `running migration: ${migration.name} on object ${rawObject.ref} (${rawObject.id})`,
    ...
    try {
      ...
      logger.error(
        `FAILED - running migration: ${migration.name} on object ${rawObject.ref} (${rawObject.id})`,
      );
      throw err;
    }
  };
...
const _updateRefsForObjects = (
  ...
  if (!index[oldRef]) {
    logger.warn(
      `the object ref: ${oldRef} has been updated to: ${newRef} but there is no reference to this object`,
    );
    // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
    return null;
  }
  ...
  if (oldRef !== newRef) {
    ...
    logger.debug(
      `replacing reference for ${realObject.id()} old ref was: ${oldRef} new ref is: ${newRef}`,
    );
    if (globalVerbose) {
      console.log(
        `replacing reference for ${realObject.id()} old ref was: ${oldRef} new ref is: ${newRef}`,
